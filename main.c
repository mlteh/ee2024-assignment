...
static void updateOledDisplay(int32_t x, int32_t y, int32_t z, int32_t value) {
	...
	if (value != prevMstVal) {
		sprintf(oledString, "X:%d Y:%d Z:%d", x, y, z);
		oled_putString(1, 10, oledString, OLED_COLOR_WHITE, OLED_COLOR_BLACK);
		sprintf(oledString, "L:%u, T:%0.1lf", light, temp);
...
int main(void) {
	...
	while (1) {
		...
		if (!btn1 && checkSW4(msTicks)) {
			if (monitorMode == 0) {
				oled_putString(1, 0, "MONITOR", OLED_COLOR_WHITE, OLED_COLOR_BLACK);
				UART_Send(LPC_UART3, (uint8_t *) msgEnterMonitor, strlen(msgEnterMonitor),
											BLOCKING);
		...
		if (monitorMode) {
			...
			if (((currentMstValue / 1000) % 16) % 5 == 0
				...
				if ((currentMstValue / 1000) % 16 == 15) {
					if (tempWarning)
						UART_Send(LPC_UART3, (uint8_t *) msgFire, strlen(msgFire),
								BLOCKING);
					if (lightWarning)
